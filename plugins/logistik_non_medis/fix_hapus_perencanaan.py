...
old_func = '''  public function postHapusPerencanaan()
  {
      ...
      if($kode_kategori) {
          $cek = $this->db('rsns_custom_logistik_non_medis_perencanaan')->where('kode_kategori', $kode_kategori)->oneArray();
          if($cek) {
              $this->db('rsns_custom_logistik_non_medis_perencanaan')->where('kode_kategori', $kode_kategori)->delete();
...
if '$_POST[\'kode_kategori\']' in old_func and old_func in c:
